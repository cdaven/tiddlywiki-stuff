module TiddlerCalendar {
    exports.name = "tiddlercalendar";
    ...
    function getDateAsNumber(d: Date): number {
        return Math.floor(
            (d.getTime() - d.getTimezoneOffset() * 60000) / MS_PER_DAY
        );
    }
    ...
    function isNormalTiddler(currentTiddler: Tiddler): boolean {
        return (
            !$tw.wiki.isSystemTiddler(currentTiddler.fields.title) &&
    ...
    function getTiddlerLookup<T extends RecordKeyTypes>(
        filter: string,
        filterFun: (tiddler: Tiddler) => boolean,
        keyFun: (tiddler: Tiddler) => T
    ): Record<T, Tiddler[]> {
    ...
    function getAllTiddlersInDateRangeByDate(
        filter: string,
        minDate: Date,
        maxDate: Date
    ): Record<number, Tiddler[]> {
    ...
    exports.run = (
        ...
        year: string = "",
        firstDayOfWeek: string = defaultFirstDayOfWeek,
        dateFormat: string = defaultDateFormat,
        weekDayFormat: string = defaultWeekDayFormat
    ): string => {
        console.log("Running macro TiddlerCalendar with arguments", {
            filter,
            year,
            firstDayOfWeek,
            dateFormat,
        ...
        const _lastDayOfWeek =
            (_firstDayOfWeek + DAYS_PER_WEEK - 1) % DAYS_PER_WEEK;
        ...
        const tiddlers = getAllTiddlersInDateRangeByDate(
            filter,
            firstDayOfTheCalendar,
            lastDayOfTheCalendar
        );
        ...
        let weekRow = [];
        for (
            let currentDay = new Date(firstDayOfTheCalendar);
            getDateAsNumber(currentDay) <= getDateAsNumber(lastDayOfTheCalendar);
            addDays(currentDay, 1)
        ) {
            const dayTiddlers = tiddlers[getDateAsNumber(currentDay)] ?? [];
            const dateString =
                getDateAsNumber(currentDay) === getDateAsNumber(today)
                    ? "<mark>TODAY</mark>"
                    : $tw.utils.formatDateString(currentDay, dateFormat);

            weekRow.push(
                `<td><p>${dateString}</p> <ul>${dayTiddlers
                    .map((tiddler) => `<li>[[${tiddler.fields.title}]]</li>`)
                    .join("")}</ul></td>`
            );
